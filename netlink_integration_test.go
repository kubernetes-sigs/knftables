...
func TestNetlinkListing(t *testing.T) {
	...
	t.Run("ListAll", func(t *testing.T) {
		objs, err := nftNL.ListAll(ctx)
		if err != nil {
			t.Fatalf("ListAll failed: %v", err)
		}
		expected := map[string][]string{
			"table":     {"knftables-test"},
			"chain":     {"mychain"},
			"set":       {"myset"},
			"map":       {"mymap"},
			"counter":   {"mycounter"},
		...
		if diff := cmp.Diff(expected, objs); diff != "" {
...
func TestNetlinkParity(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		t.Run(fmt.Sprintf("Iteration-%d", i), func(t *testing.T) {
			...
			// Verify ListAll
			allNetlink, err := clientNetlink.ListAll(ctx)
			if err != nil {
				t.Fatalf("Netlink ListAll failed: %v", err)
			}
			allBinary, err := clientBinary.ListAll(ctx)
			if err != nil {
				t.Fatalf("Binary ListAll failed: %v", err)
			}
			if diff := cmp.Diff(allBinary, allNetlink, cmpopts.SortSlices(func(a, b string) bool { return a < b })); diff != "" {
				t.Errorf("ListAll mismatch (-binary +netlink):\n%s", diff)
			}
		})
	}
}
